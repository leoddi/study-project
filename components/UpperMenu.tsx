    ...
    <div className="w-full bg-transparent flex justify-between fixed z-50 lg:hidden">
      <div className="flex justify-between lg:hidden space-x-1">
        ...
        <div className="h-12 w-12 bg-[#A68547] flex items-center justify-center cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1" stroke="white" className="size-8">
            <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
          </svg>
        </div>
      </div>
      <div onClick={()=>{setshowMenu(!showMenu); setmenuNav("")}} className={`${showMenu? 'bg-white':'bg-[#4A362A]'} h-12 w-12`}>
        {showMenu? 
          ...
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="white" className="size-auto m-2">
    ...
    {showMenu?
      <div className="w-full h-full fixed lg:hidden bg-[#4A362A] pt-18 px-10 z-30 text-white">
        <ul className="text-lg uppercase text-[#AA9384] tracking-wide mb-8">
          {links.map((link, i)=>(
            <div key={i}>
              <li className="w-min text-nowrap hover:text-[#A68547] cursor-pointer" onClick={() => setmenuNav(link as MenuRoute)} onMouseLeave={()=>setmenuNav("")} >{link.replace(/([A-Z])/g, ' $1').toLowerCase()}</li>
